<?php
	...
	function getTempCodeAndFile($url){
		$positionsOfSlashes = array();
		for($i = 0;$i < strlen($url);$i++){
			$char = substr($url, $i, 1);
			if($char == "/"){
				array_push($positionsOfSlashes, $i);
			}
		}
		if(count($positionsOfSlashes) < 2){
			// die("EADsa");
			die(error("Error: Unable to find temp file."));
		}
		rsort($positionsOfSlashes);

		$data = array();
		$filename = substr($url, $positionsOfSlashes[0]);
		$filename = str_replace("/","",$filename);
		$temp = substr($url, $positionsOfSlashes[1]);
		$temp = str_replace($filename, "", $temp);
    ...
    if($uploadedFilePath == ""){
    	die(error("Error: Missing file"));
    }


    $d = getTempCodeAndFile($uploadedFilePath);
    $temp = $d['temp'];
    $filename = $d['filename'];
	...
	if($pageCount > 8){
		...
		for($i = 1;$i <= $pageCount;$i++){
			try {
				...
				$pdf = new PDFMerger;
				$pdf->addPDF($uploadedFilePath, (strval($i) . '-' . strval($i)));
